...
/// <summary>
/// Extension methods for registering MSAL services in the DI container.
/// </summary>
internal static class MsalServiceExtensions
{
    /// <summary>
    /// Registers the MSAL public client application and <see cref="MsalTokenService"/> in the DI container.
    /// </summary>
    public static IServiceCollection AddMsalClient(this IServiceCollection services)
    {
        var config = MsalConfig.Instance;
...
#if WINDOWS
        msalBuilder.WithWindowsEmbeddedBrowserSupport();
#endif
    ...
    /// automatically persist (Windows, Mac Catalyst).
    /// </summary>
    ...
    /// On Android, sets the parent activity. On iOS, configures system web view.
    /// On Mac Catalyst, uses ASWebAuthenticationSession workaround.
    /// On Windows, sets the WinUI3 parent window.
    /// </summary>
    public static AcquireTokenInteractiveParameterBuilder WithPlatformOptions(
        this AcquireTokenInteractiveParameterBuilder builder)
    {
        ...
        // Remove this once MSAL ships Mac Catalyst support:
        ...
        if (app.Windows.FirstOrDefault() is not IWindow win
            || win.Handler?.PlatformView is not Microsoft.UI.Xaml.Window platformWindow)
